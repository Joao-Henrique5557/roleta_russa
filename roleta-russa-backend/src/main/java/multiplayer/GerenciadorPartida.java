...
public class GerenciadorPartida {
	...
	public GerenciadorPartida(ClienteHandler p1, ClienteHandler p2) {
		...
		this.revolver.rearmazenarBalas();
		this.vez = p1;
		iniciar();
	}
	...
	private void iniciar() {
		...
		p1.getJogador().setVidas(3);
		p1.getJogador().setAlive(true);
		p2.getJogador().setVidas(3);
		p2.getJogador().setAlive(true);
		notificarTurno();
	}
	...
	public synchronized void processarAcao(ClienteHandler autor, int escolha) {
		...
		String msg = Protocolo.montar(Protocolo.RESULTADO, alvo.getNome(),
				alvo.isAlive() ? (disparou ? "ATINGIDO" : "SALVO") : "MORTO", String.valueOf(alvo.getVidas()));
		broadcast(msg);

		if (!p1.getJogador().isAlive() || !p2.getJogador().isAlive()) {
			finalizar();
		} else {
			// Se atirou em si mesmo e não morreu/não disparou, mantém a vez. Caso contrário
			// troca.
	...
	private void notificarTurno() {
		if (revolver.getQuantBalas() == 0)
			revolver.rearmazenarBalas();

		broadcast(Protocolo.montar(Protocolo.INFO_REVOLVE, String.valueOf(revolver.getQuantBalasRestantes()),
	...
	private ClienteHandler oponenteDe(ClienteHandler c) {
		return (c == p1) ? p2 : p1;
	}
}
